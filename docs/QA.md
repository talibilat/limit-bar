...
| Codex quota explanation | `CodexQuotaExplanationTests` verifies latest compatible pair selection inside one Quota window with one Exact boundary, deterministic primary/session preference, separation of measured quota movement from Observed Local Breakdown tokens, explicit unattributed status, incomplete coverage as partial, evidence identities that include the privacy-safe session digest, and distinct unavailable states for gaps, observed zero local activity, counter decreases, no positive quota movement, and quota-window reset. Debug UI automation exposes the explanation fixture through `LIMITBAR_UI_TEST_SCREEN=codex-explanation` without reading real Codex files. |
