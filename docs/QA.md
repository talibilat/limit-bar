...
| Quota evidence diagnostic export | `DiagnosticExportTests` snapshots the schema v6 positive allow-list with bounded exact selection metadata, movement provenance, local aggregate counts, unattributed movement, forecasts, anomalies, versions, limitations, and privacy-safe trace references; prohibited sentinels verify no raw payloads, paths, account labels, event content, private identifiers, or unapproved fields are exported. |
