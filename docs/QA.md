...
| Codex explanation persistence | `CodexExplanationStoreTests` verifies `codex-explanations.sqlite` stores only bounded normalized findings, survives reopen, prunes by age and count transactionally, rejects future and malformed schemas without mutation, and deletes independently. `ProviderRefreshHistoryPresentationTests` verifies deletion preserves current usage, quota observations, settings, credentials, alert rules, and notification delivery history. Migration fixtures include synthetic pre-release and first-release Codex explanation schemas. |
| Codex diagnostic export | `DiagnosticExportTests` snapshots schema v5 with coarse Codex explanation status, coverage, token-evidence category, bounded counts, barrier categories, and adapter version; prohibited sentinels verify no raw payloads, paths, token values, exact windows, resets, session IDs, or digests are exported. |
...
37. With a signed app and synthetic Codex 0.144.4 rollout fixture under a configured sessions boundary, confirm the Codex row shows measured quota movement, Observed Local Breakdown tokens, adapter `codex-rollout-observed-0.144.4`, and an explicit statement that quota movement remains unattributed.
