...
| Quota evidence diagnostic export | `DiagnosticExportTests` snapshots the schema v6 positive allow-list with bounded exact selection metadata, movement provenance, local aggregate counts, typed unattributed remainder, forecasts, anomalies, closed methods and reasons, strict safe-token versions, limitations, and privacy-safe trace references; shuffled sets above the projection cap verify byte stability and complete omitted counts, while prohibited sentinels verify no raw payloads, paths, account labels, event content, private identifiers, or unapproved fields are exported. |
