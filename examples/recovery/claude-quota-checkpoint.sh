...
# Raw hook JSON, errors, prompts, and output are never forwarded to LimitBar.
: "${LIMITBAR_SESSION_REFERENCE:?opaque Claude Code session reference is required}"
: "${LIMITBAR_RESET_BOUNDARY:?provider-reported ISO 8601 reset boundary is required}"
: "${CLAUDE_CODE_VERSION:?Claude Code version is required}"
