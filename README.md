...
The Analysis tab labels provider inputs as **Measured** and derived ranges as **Calculated**; there is no additional gauge or dashboard.
These findings do not drive notifications, so ticket 12 alert qualification and measured-only behavior remain unchanged.

## Planned Workloads

The Analysis tab includes a concise local, ephemeral planning assessment.
