...
Profiling and CI artifacts must contain aggregate timings and statuses only.
No raw prompts, code, responses, terminal output, credentials, or raw provider payloads may be captured.

## Data Model Impact

No product data-model change is required for signing, CI, or profiling.
...
- Native UI automation covers launch, passive Claude state, Connect affordance, and local source configuration.
